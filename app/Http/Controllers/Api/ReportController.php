...
class ReportController extends Controller
{
    ...
    // Helper method to apply vendor filter
    private function applyVendorFilter($query, $user)
    {
        if ($user->isVendor()) {
    ...
    // Helper method to apply vendor filter to product queries
    ...
    public function salesOverview(Request $request)
    {
        ...
        $orderQuery = $this->applyVendorFilter($orderQuery, $user);

        $totalRevenue = $orderQuery->sum('total');
        $totalOrders = $orderQuery->count();
        
        // For new customers, vendors can only see customers who bought their products
        $newCustomersQuery = User::whereHas('orders', function($q) use ($start, $end, $user) {
            $q->whereBetween('created_at', [$start, $end]);
            if ($user->isVendor()) {
                $q->whereHas('items.product', function($pq) use ($user) {
        ...
        // Previous period queries with vendor filter
        $prevOrderQuery = Order::whereBetween('created_at', [$prevStart, $prevEnd]);
        $prevOrderQuery = $this->applyVendorFilter($prevOrderQuery, $user);

        $prevRevenue = $prevOrderQuery->sum('total');
        $prevOrders = $prevOrderQuery->count();
        
        $prevCustomersQuery = User::whereHas('orders', function($q) use ($prevStart, $prevEnd, $user) {
        ...
        $prevCustomers = $prevCustomersQuery->count();
    ...
    public function salesReport(Request $request)
    {
        ...
        $salesQuery = $this->applyVendorFilter($salesQuery, $user);
    ...
    public function productPerformance(Request $request)
    {
        try {
            ...
            // Apply vendor filter to products
            $productQuery = $this->applyVendorProductFilter($productQuery, $user);

            $products = $productQuery->withCount(['orderItems as units_sold' => function($q) use ($start, $end, $user) {
    ...
    public function inventoryReport(Request $request)
    {
        ...
        $inventory = $products->map(function($p) use ($lowStockThreshold) {
            $status = 'In Stock';
            $action = 'None';
    ...
    public function customerAnalysis(Request $request)
    {
        try {
            ...
            // For vendors, only show customers who bought their products
            if ($user->isVendor()) {
            ...
            $customers = $customerQuery->withCount(['orders as total_orders' => function($q) use ($start, $end, $user) {
                    $q->whereBetween('created_at', [$start, $end]);
                    if ($user->isVendor()) {
                        $q->whereHas('items.product', function($pq) use ($user) {
                            $pq->where('vendor_id', $user->id);
                ...
                ->withSum(['orders as total_revenue' => function($q) use ($start, $end, $user) {
                    $q->whereBetween('created_at', [$start, $end]);
                    if ($user->isVendor()) {
                        $q->whereHas('items.product', function($pq) use ($user) {
                            $pq->where('vendor_id', $user->id);
                ...
                ->with(['orders' => function($q) use ($start, $end, $user) {
                    $q->whereBetween('created_at', [$start, $end]);
                    if ($user->isVendor()) {
                        $q->whereHas('items.product', function($pq) use ($user) {
                            $pq->where('vendor_id', $user->id);
    ...
    public function exportReport(Request $request, $type)
    {
        try {
            ...
            // Add vendor-specific filename
            if ($user->isVendor()) {
                $filename .= '_vendor_' . $user->id;
            }
            ...
            if ($type === 'csv') {
                ...
                $rowNumber = 1;
                foreach ($data as $row) {
                    $csv->insertOne(array_merge([$rowNumber], array_values((array)$row)));
                    $rowNumber++;
                }
